 ...
 * @param fg 圆环边框色，为nullptr表示不绘制边框
 */
void Draw_Circle_middle(float centerx, float centery, float radius, float width, SDL_Color* bg, SDL_Color* fg)
{
    Draw_Circle(centerx, centery, radius + width / 2, width, bg, fg);
}
...
{
    Draw_Circle(centerx, centery, radius + width / 2, topleft_radius, topright_radius, bottomleft_radius, bottomright_radius, width, bg, fg);
}
...
void Draw_Rect(SDL_FRect rect, float width, SDL_Color* bg, SDL_Color* fg, float topleft_radius, float topright_radius, float bottomleft_radius, float bottomright_radius)
{
    ...
    {
        Draw_Circle_middle(rect.x + topleft_radius, rect.y + topleft_radius, topleft_radius, true, false, false, false, width, bg, fg);                                  // 左上角圆
        Draw_Circle_middle(rect.x + rect.w - topright_radius, rect.y + topright_radius, topright_radius, false, true, false, false, width, bg, fg);                      // 右上角圆
        Draw_Circle_middle(rect.x + bottomleft_radius, rect.y + rect.h - bottomleft_radius, bottomleft_radius, false, false, true, false, width, bg, fg);                // 左下角圆
        Draw_Circle_middle(rect.x + rect.w - bottomright_radius, rect.y + rect.h - bottomright_radius, bottomright_radius, false, false, false, true, width, bg, fg);    // 右下角圆
    }
...
{
    if (x < 0 || x >= surface->w || y < 0 || y >= surface->h) return {0, 0, 0, 0};

    Uint32* pixels = static_cast<Uint32*>(surface->pixels);
    Uint32 pixel_color = pixels[y * surface->w + x];

    SDL_Color color;
    SDL_GetRGBA(pixel_color, SDL_GetPixelFormatDetails(surface->format), NULL, &color.r, &color.g, &color.b, &color.a);
    return color;
}
...
void set_surface_pixel(SDL_Surface* surface, int x, int y, SDL_Color color)
{
    ...
    Uint32* pixels = static_cast<Uint32*>(surface->pixels);
    Uint32 pixel_color = SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, color.r, color.g, color.b, color.a);
 ...
 * @brief 设置多个像素点颜色
 * @param surface SDL_Surface指针
 * @param points 像素点坐标列表
 * @param color 颜色
 */
void set_surface_pixels(SDL_Surface* surface, const SDL_FPoint *points, SDL_Color color, int count)
{
    if (!surface || !points) return;
 ...
 * @param fg 圆环边框色，为nullptr表示不绘制边框
 */
void Draw_Circle_Surface(SDL_Surface* surface, float centerx, float centery, float radius, float width, SDL_Color* bg, SDL_Color* fg)
{
    // 计算填充半径
    float fill_radius = std::max(radius - width, 0.0f);
    ...
    if (bg != nullptr && fill_radius > 0)
    {
        ...
        if (!points.empty())
        {
            set_surface_pixels(surface, points.data(), *bg, static_cast<int>(points.size()));
        }
    }
    ...
    if (fg != nullptr && width > 0)
    {
        ...
        if (!border_points.empty())
        {
            set_surface_pixels(surface, border_points.data(), *fg, static_cast<int>(border_points.size()));
        }
    }
}
...
void Draw_Circle_Surface(SDL_Surface* surface, float centerx, float centery, float radius, bool topleft_radius, bool topright_radius, bool bottomleft_radius, bool bottomright_radius, float width, SDL_Color* bg, SDL_Color* fg)
{
    ...
    if (!fill_points.empty())
    {
        set_surface_pixels(surface, fill_points.data(), *bg, static_cast<int>(fill_points.size()));
    }
    if (!border_points.empty())
    {
        set_surface_pixels(surface, border_points.data(), *fg, static_cast<int>(border_points.size()));
...
void Draw_Rect_Surface(SDL_Surface* surface, SDL_FRect rect, float width, SDL_Color* bg, SDL_Color* fg)
{
    ...
    {
        SDL_FRect border_rects[4] = {
            {rect.x, rect.y, rect.w, width}, // 上边框
            {rect.x, rect.y + rect.h - width, rect.w, width}, // 下边框
            {rect.x, rect.y + width, width, rect.h - width * 2}, // 左边框
            {rect.x + rect.w - width, rect.y + width, width, rect.h - width * 2}  // 右边框
        };
        SDL_SetRenderDrawColor(renderer, fg->r, fg->g, fg->b, fg->a);
        SDL_RenderFillRects(renderer, border_rects, 4);
        rect.w -= width * 2;
    ...
    if (bg != nullptr)
    {
        SDL_Rect sdl_rect = { static_cast<int>(rect.x), static_cast<int>(rect.y), static_cast<int>(rect.w), static_cast<int>(rect.h) };
        SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, bg->r, bg->g, bg->b, bg->a));
    }
}

/**
 * @brief 绘制圆角矩形到SDL_Surface
 * @param surface SDL_Surface指针
 * @param rect 矩形区域
 * @param width 矩形边框宽度，为0表示不绘制边框
 * @param bg 矩形填充色，为nullptr表示不填充
...
void Draw_Rect_Surface(SDL_Surface* surface, SDL_FRect rect, float width, SDL_Color* bg, SDL_Color* fg, float radius)
{
    ...
    const float max_radius = std::min(rect.w, rect.h) / 2;
    radius = std::min(radius, max_radius);
    Draw_Rect_Surface(surface, rect, width, bg, fg, radius, radius, radius, radius);
}
...
void Draw_Rect_Surface(SDL_Surface* surface, SDL_FRect rect, float width, SDL_Color* bg, SDL_Color* fg, float topleft_radius, float topright_radius, float bottomleft_radius, float bottomright_radius)
{
    ...
    if (need_fill || need_border)
    {
        Draw_Circle_Surface(surface, rect.x + topleft_radius, rect.y + topleft_radius, topleft_radius, true, false, false, false, width, bg, fg);                                  // 左上角圆
        Draw_Circle_Surface(surface, rect.x + rect.w - topright_radius, rect.y + topright_radius, topright_radius, false, true, false, false, width, bg, fg);                      // 右上角圆
        Draw_Circle_Surface(surface, rect.x + bottomleft_radius, rect.y + rect.h - bottomleft_radius, bottomleft_radius, false, false, true, false, width, bg, fg);                // 左下角圆
        Draw_Circle_Surface(surface, rect.x + rect.w - bottomright_radius, rect.y + rect.h - bottomright_radius, bottomright_radius, false, false, false, true, width, bg, fg);    // 右下角圆
    }
    ...
    if (need_fill)
    {
        ...
        if (center_rect.w > 0 && center_rect.h > 0)
        {
            SDL_Rect sdl_rect = { static_cast<int>(center_rect.x), static_cast<int>(center_rect.y), static_cast<int>(center_rect.w), static_cast<int>(center_rect.h) };
            SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, bg->r, bg->g, bg->b, bg->a));
        }
        ...
        if (top_rect.h > 0 && top_rect.w > 0)
        {
            SDL_Rect sdl_rect = { static_cast<int>(top_rect.x), static_cast<int>(top_rect.y), static_cast<int>(top_rect.w), static_cast<int>(top_rect.h) };
            SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, bg->r, bg->g, bg->b, bg->a));
        }
        if (bottom_rect.h > 0 && bottom_rect.w > 0)
        {
            SDL_Rect sdl_rect = { static_cast<int>(bottom_rect.x), static_cast<int>(bottom_rect.y), static_cast<int>(bottom_rect.w), static_cast<int>(bottom_rect.h) };
            SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, bg->r, bg->g, bg->b, bg->a));
        }
        ...
        if (left_rect.w > 0 && left_rect.h > 0)
        {
            SDL_Rect sdl_rect = { static_cast<int>(left_rect.x), static_cast<int>(left_rect.y), static_cast<int>(left_rect.w), static_cast<int>(left_rect.h) };
            SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, bg->r, bg->g, bg->b, bg->a));
        }
        if (right_rect.w > 0 && right_rect.h > 0)
        {
            SDL_Rect sdl_rect = { static_cast<int>(right_rect.x), static_cast<int>(right_rect.y), static_cast<int>(right_rect.w), static_cast<int>(right_rect.h) };
            SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, bg->r, bg->g, bg->b, bg->a));
        }
    }
    ...
    if (need_border) {
        ...
        SDL_Rect sdl_rect = { static_cast<int>(top_border.x), static_cast<int>(top_border.y), static_cast<int>(top_border.w), static_cast<int>(top_border.h) };
        SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, fg->r, fg->g, fg->b, fg->a));
        sdl_rect = { static_cast<int>(bottom_border.x), static_cast<int>(bottom_border.y), static_cast<int>(bottom_border.w), static_cast<int>(bottom_border.h) };
        SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, fg->r, fg->g, fg->b, fg->a));
        sdl_rect = { static_cast<int>(left_border.x), static_cast<int>(left_border.y), static_cast<int>(left_border.w), static_cast<int>(left_border.h) };
        SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, fg->r, fg->g, fg->b, fg->a));
        sdl_rect = { static_cast<int>(right_border.x), static_cast<int>(right_border.y), static_cast<int>(right_border.w), static_cast<int>(right_border.h) };
        SDL_FillSurfaceRect(surface, &sdl_rect, SDL_MapRGBA(SDL_GetPixelFormatDetails(surface->format), NULL, fg->r, fg->g, fg->b, fg->a));
    }
}
