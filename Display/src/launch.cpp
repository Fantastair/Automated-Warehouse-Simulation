...
void Launch_Init(void)
{
    ...
    title->set_rect_midleft(144, icon->rect.y + 32);
    title->join(root);
    RectUi *line = new RectUi(550, 8, 0, rm.getColor_(DARKBLUE), nullptr, 4);
    line->set_rect_midleft(144, icon->rect.y + 60);
    line->join(root);
    ...
    subtitle1->set_rect_midleft(144, icon->rect.y + 84);
    subtitle1->join(root);
    subtitle2->set_rect_midleft(144, icon->rect.y + 108);
    subtitle2->join(root);
